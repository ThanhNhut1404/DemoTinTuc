...
<div class="card">
    ...
    <form id="searchForm" class="form-inline" method="get" action="admin.php" style="margin-bottom:12px;position:relative;">
        ...
        <button type="button" id="filterToggle" class="btn btn-filter" aria-expanded="false">Bộ lọc</button>
        ...
        <div id="filterPanel" class="filter-panel" role="region" aria-hidden="true">
            ...
            <h4 class="filter-title" style="margin:0 0 8px 0;">Bộ lọc</h4>
            <fieldset>
                <legend>Quyền</legend>
                <select name="role">
            ...
            <div style="margin-top:8px;text-align:right;">
                ...
                <a href="admin.php?action=index" style="margin-left:8px;" class="btn btn-clear-filter">Xóa bộ lọc</a>
            </div>
        </div>
    </form>
            ...
            <?php foreach ($dsThanhVien as $tv): ?>
                <tr>
                    ...
                    <td class="col-actions">
                        ...
                        <?php else: ?>
                            <a class="btn btn-unlock" href="admin.php?action=unlock&id=<?= urlencode($tv['id']) ?>&hanhDong=mo<?= !empty($_GET['role']) ? '&role=' . urlencode($_GET['role']) : '' ?><?= !empty($_GET['status']) ? '&status=' . urlencode($_GET['status']) : '' ?><?= !empty($_GET['gender']) ? '&gender=' . urlencode($_GET['gender']) : (!empty($_GET['gioi_tinh']) ? '&gender=' . urlencode($_GET['gioi_tinh']) : '') ?>">Mở</a>
                        <?php endif; ?>
